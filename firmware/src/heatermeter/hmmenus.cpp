...
// scratch space for edits (must hold a full probe name = PROBE_NAME_SIZE)
int editInt;
char editString[PROBE_NAME_SIZE];
...
// Horizontal-scroll state for the rotating home line (probe names longer than
// the 11-column name field). Time-based (millis) so the speed is independent of
// the display refresh rate, with a leading pause so the start of the name is
// always shown when a probe is switched in. Resets when the probe changes.
static unsigned long g_homeScrollStart = 0;
static unsigned char g_homeScrollProbe = 0xff;
#define HOME_SCROLL_LEAD_MS 1500UL   // hold at the start of the name
#define HOME_SCROLL_STEP_MS 350UL    // then advance one column per step
...
static void updateHome(void)
{
  ...
  for (unsigned char probeIndex = probeIdxLow; probeIndex <= probeIdxHigh; ++probeIndex)
  {
    if (probeIndex < TEMP_COUNT && pid.Probes[probeIndex]->hasTemperature())
    {
      ...
      // Name in the left 11 cols, a space, then the right 4 are the temperature.
      // Names longer than 11 scroll horizontally.
      char nbuf[12];
      unsigned char nlen = strlen(editString);
      if (nlen > 11)
      {
        // Restart the scroll (with its leading pause) whenever the probe changes
        // so the start of the name is always visible right after a switch.
        if (g_homeScrollProbe != probeIndex)
        {
          g_homeScrollProbe = probeIndex;
          g_homeScrollStart = millis();
        }
        unsigned char span = nlen + 2;   // 2-space gap before the name wraps
        unsigned long el = millis() - g_homeScrollStart;
        unsigned char off = (el < HOME_SCROLL_LEAD_MS) ? 0
          : (unsigned char)(((el - HOME_SCROLL_LEAD_MS) / HOME_SCROLL_STEP_MS) % span);
        for (unsigned char k = 0; k < 11; ++k)
        {
          unsigned char s = (off + k) % span;
          nbuf[k] = (s < nlen) ? editString[s] : ' ';
        }
      }
      else
      {
        for (unsigned char k = 0; k < 11; ++k) nbuf[k] = (k < nlen) ? editString[k] : ' ';
        g_homeScrollProbe = 0xff;
      }
      nbuf[11] = '\0';
      snprintf_P(buffer, sizeof(buffer), PSTR("%s %3d" DEGREE), nbuf,
...
unsigned char g_homeRotateSecs = 5;  // default per-probe dwell on the home screen
void setHomeRotateSecs(unsigned char secs)
{
  if (secs < 1) secs = 1;
  if (secs > 60) secs = 60;
  g_homeRotateSecs = secs;
  Menus.setRotateTimeout(ST_HOME_FOOD1, ST_HOME_FOOD3, secs);
}
