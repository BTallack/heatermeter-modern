...
void GrillPid::reportStatus(void) const
{
  ...
  // Field 11 (appended): PID mode 0=Startup 1=Recovery 2=Normal 3=Manual 4=Off.
  // Lets the host distinguish "coming up to temp" from "at temp" / "recovering".
  // Backward-compatible: older host parsers simply ignore the extra field.
  Serial_csv();
  SerialX.print(getPidMode(), DEC);
  Serial_nl();
