...
// 96 (was 64): a /set?pc with four custom Steinhart coefficients plus the type
// can approach the old limit, and an overflow is silently discarded. RAM is
// plentiful, so give the line buffer comfortable headroom.
static char g_SerialBuff[96];
...
// Bumped 0xf00e -> 0xf00f: PROBE_NAME_SIZE grew (13->23), changing the probe
// config stride, so the stored layout must re-initialise. The host backs up +
// restores all settings (names, probe coeffs, PID, fan, alarms, offsets) via
// the API around the flash, so nothing is actually lost.
#define EEPROM_MAGIC 0xf00f
...
static void handleCommandUrl(char *URL)
{
  ...
  else if (strncmp_P(URL, PSTR("set?pc"), 6) == 0 && urlLen > 7)
  {
    ...
    reportProbeCoeffs();  // echo back like every other /set? command
  }
  else if (strncmp_P(URL, PSTR("set?hr="), 7) == 0)
  {
    // Home-screen probe rotation interval (seconds). RAM only; the host
    // re-sends it on connect so it survives a board reboot.
    setHomeRotateSecs(atoi(URL + 7));
  }
...
#ifdef HEATERMETER_SERIAL
/* hm4: validate an optional trailing XOR checksum on inbound command lines
   ("/set?sp=225*1F"). The host appends '*' plus two hex digits: the XOR of
   every byte of the line before the '*'. A line carrying a checksum that is
   malformed or does not match is DROPPED whole - this rejects lines garbled
   in transit, e.g. two commands merged by a lost newline (which once stored a
   command string as a probe name). Lines without a '*' suffix are accepted
   unchanged, so stock tools and manual terminals keep working. On success the
   checksum is stripped before dispatch. Returns nonzero if ok to dispatch. */
static unsigned char serial_validateCsum(void)
{
  char *star = strrchr(g_SerialBuff, '*');
  unsigned char want, csum, i;
  char *p;
  if (star == NULL)
    return 1;
  /* exactly two hex digits then end-of-line */
  want = 0;
  for (i = 1; i <= 2; ++i)
  {
    char c = star[i];
...
static void serial_doWork(void)
{
  ...
  while (Serial.available())
  {
    ...
    if (c == '\n' || c == '\r')
    {
      if (len != 0 && g_SerialBuff[0] == '/' && serial_validateCsum())
...
void hmcoreSetup(void)
{
  ...
  // Arm the hardware watchdog now that setup is complete. The main loop is fully
  // cooperative (no blocking delays), so it kicks the dog every pass; if any
  // handler ever wedges, the AVR resets within ~8s and recovers on its own
  // (important for an unattended cooker controlling a blower over live fire).
  // .init3 (heatermeter.ino) clears MCUSR + the WDT on every reset, and there is
  // no bootloader, so the WDT-reset path boots straight back into the app.
  // /reboot still works: it sets a 30ms WDT timeout which fires first.
  wdt_enable(WDTO_8S);
}

void hmcoreLoop(void)
{
  wdt_reset();  // pet the watchdog armed in hmcoreSetup()
