...
public:
  ...
  // Override the state timeout for a contiguous range of states (used to make
  // the home-screen probe rotation interval runtime-configurable). secs=0 off.
  void setRotateTimeout(state_t lo, state_t hi, unsigned char secs)
  { m_rotateLo = lo; m_rotateHi = hi; m_rotateSecs = secs; }
private:
  ...
  state_t m_rotateLo;
  state_t m_rotateHi;
  unsigned char m_rotateSecs;  // 0 = use the per-state table timeout
