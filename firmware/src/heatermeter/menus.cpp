...
MenuSystem::MenuSystem(const menu_definition_t *defs, const menu_transition_t *trans,
  ...
  : m_definitions(defs), m_transitions(trans), m_readButton(reader),
    m_rotateLo(0), m_rotateHi(0), m_rotateSecs(0)
...
{
  // Runtime override for a state range (home-screen probe rotation interval).
  if (m_rotateSecs && m_state >= m_rotateLo && m_state <= m_rotateHi)
    return (unsigned long)m_rotateSecs * 1000;
